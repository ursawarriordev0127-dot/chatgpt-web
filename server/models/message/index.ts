...
async function addMessages(datas: Array<{ [key: string]: any }>) {
  ...
  const sanitizedDatas = datas.map(data => {
    const sanitized = { ...data }
    // Convert empty strings to null for BIGINT fields
    if (sanitized.persona_id === '' || sanitized.persona_id === 'null' || sanitized.persona_id === 'undefined') {
      sanitized.persona_id = null
    }
    if (sanitized.plugin_id === '' || sanitized.plugin_id === 'null' || sanitized.plugin_id === 'undefined') {
      sanitized.plugin_id = null
    }
    if (sanitized.parent_message_id === '' || sanitized.parent_message_id === 'null' || sanitized.parent_message_id === 'undefined') {
      sanitized.parent_message_id = null
    }
    return sanitized
  })
  const captains = await messageMysql.bulkCreate([...sanitizedDatas])
  return captains
}
